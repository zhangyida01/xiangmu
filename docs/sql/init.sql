...
CREATE TABLE sys_user (
  ...
  eal_name VARCHAR(50) COMMENT '真实姓名',
  email VARCHAR(100) COMMENT '邮箱',
  phone VARCHAR(20) COMMENT '手机号',
  dept_id BIGINT COMMENT '部门ID',
  status TINYINT DEFAULT 1 COMMENT '状态：0-禁用 1-启用',
  vatar VARCHAR(255) COMMENT '头像',
...
CREATE TABLE sys_role (
  ...
  ole_code VARCHAR(50) NOT NULL COMMENT '角色编码',
  ole_name VARCHAR(50) NOT NULL COMMENT '角色名称',
...
CREATE TABLE sys_user_role (
  ...
  ole_id BIGINT NOT NULL COMMENT '角色ID',
  PRIMARY KEY (id),
...
CREATE TABLE project_document (
  ...
  ile_path VARCHAR(500) COMMENT '文件路径',
  ile_size BIGINT COMMENT '文件大小（字节）',
  ersion VARCHAR(20) DEFAULT '1.0' COMMENT '版本号',
...
CREATE TABLE project_task (
  ...
  parent_id BIGINT DEFAULT 0 COMMENT '父任务ID',
  	ask_name VARCHAR(200) NOT NULL COMMENT '任务名称',
  ssignee_id BIGINT COMMENT '负责人ID',
  start_date DATE COMMENT '开始日期',
  end_date DATE COMMENT '结束日期',
...
CREATE TABLE project_issue (
  ...
  issue_code VARCHAR(50) COMMENT '问题编号',
  	itle VARCHAR(200) NOT NULL COMMENT '问题标题',
  description TEXT COMMENT '问题描述',
  issue_type TINYINT DEFAULT 1 COMMENT '类型：1-问题 2-风险',
  priority TINYINT DEFAULT 2 COMMENT '优先级：1-高 2-中 3-低',
  status TINYINT DEFAULT 0 COMMENT '状态：0-待处理 1-处理中 2-已解决 3-已关闭',
  eporter_id BIGINT COMMENT '提报人ID',
  ssignee_id BIGINT COMMENT '处理人ID',
  esolve_time DATETIME COMMENT '解决时间',
  create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
  create_by BIGINT,
  update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  update_by BIGINT,
...
CREATE TABLE project_change (
  ...
  change_code VARCHAR(50) COMMENT '变更编号',
  	itle VARCHAR(200) NOT NULL COMMENT '变更标题',
  ...
  pplicant_id BIGINT COMMENT '申请人ID',
  pprover_id BIGINT COMMENT '审批人ID',
  pprove_time DATETIME COMMENT '审批时间',
  create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
  create_by BIGINT,
  update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  update_by BIGINT,
...
CREATE TABLE project_acceptance (
  ...
  project_id BIGINT NOT NULL COMMENT '项目ID',
  cceptance_code VARCHAR(50) COMMENT '验收单编号',
  	itle VARCHAR(200) NOT NULL COMMENT '验收标题',
  cceptance_date DATE COMMENT '验收日期',
  customer_sign VARCHAR(50) COMMENT '客户签字人',
  status TINYINT DEFAULT 0 COMMENT '状态：0-待验收 1-已通过 2-未通过',
  esult TEXT COMMENT '验收结果',
  create_time DATETIME DEFAULT CURRENT_TIMESTAMP,
  create_by BIGINT,
  update_time DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  update_by BIGINT,
...
CREATE TABLE service_ticket (
  id BIGINT NOT NULL AUTO_INCREMENT COMMENT '工单ID',
  	icket_code VARCHAR(50) NOT NULL COMMENT '工单编号',
  project_id BIGINT NOT NULL COMMENT '项目ID',
  	itle VARCHAR(200) NOT NULL COMMENT '工单标题',
  description TEXT COMMENT '问题描述',
  priority TINYINT DEFAULT 2 COMMENT '优先级：1-高 2-中 3-低',
  status TINYINT DEFAULT 0 COMMENT '状态：0-待处理 1-处理中 2-已解决 3-已关闭',
  eporter_id BIGINT COMMENT '提交人ID',
  ssignee_id BIGINT COMMENT '处理人ID',
  ...
  deleted TINYINT DEFAULT 0,
  PRIMARY KEY (id),
  UNIQUE KEY uk_ticket_code (	icket_code)
